  ...
  post: async (req, res) => {
    try {
      ...
      if (tokenResponse.ok) {
        ...
        const linkResponse: LinkTokenCreateResponse = await linkTokenResponse.json();
        console.log(linkResponse)
        res.status(linkTokenResponse.status).json(linkResponse);
      }
    } catch (error: any) {
      console.error(error);
